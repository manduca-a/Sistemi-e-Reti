...
class BanconeBar:
    ...
    def miglioraPosizione(self, r, c):
        with self.lock:
            ...
            if 0 <= c < self.colonne and 0 <= r < len(self.bancone[c]):
                colonnaMigliore = c

                if c > 0 and len(self.bancone[c-1]) + 1 < len(self.bancone[colonnaMigliore]):       #se la colonna c-1 incrementata di 1 elemento è comunque più corta della colonna c
                    colonnaMigliore = c-1
                if c < self.colonne - 1 and len(self.bancone[c+1]) + 1 < len(self.bancone[colonnaMigliore]):     #se la colonna c+1 incrementata di 1 elemento è comunque più corta della colonna migliore
                    colonnaMigliore = c+1

                if colonnaMigliore != c:
                    elemento = self.bancone[c].pop(r)
                    self.bancone[colonnaMigliore].append(elemento)
                    print(f"elemento spostato da [{c}][{r}] a {colonnaMigliore}")
...
def inserisci_elementi(bancone):
    while True:
        ...
        if (random.random() > 0.7):
            colonna = random.randint(0, 10)
            riga = random.randint(0, 10)
            print(f"provo a spostare! {colonna, riga} se esiste")
            bancone.miglioraPosizione(colonna, riga)
